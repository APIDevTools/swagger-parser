...
describe("Invalid APIs (Swagger 2.0 specification validation)", () => {
  ...
  it('should pass validation if "options.validate.spec" is false', async () => {
    ...
    const api = await SwaggerParser.validate(path.rel("test/specs/validate-spec/invalid/" + invalid.file), {
      validate: { spec: false },
    });
  ...
  for (const test of tests) {
    if (test.valid) {
      it(test.name, async () => {
        try {
          const api = await SwaggerParser.validate(path.rel("test/specs/validate-spec/valid/" + test.file));
          expect(api).to.be.an("object");
        } catch (err: any) {
      ...
      it(test.name, async () => {
        try {
          await SwaggerParser.validate(path.rel("test/specs/validate-spec/invalid/" + test.file));
