...
describe("Invalid APIs (Swagger 2.0 schema validation)", () => {
  ...
  it('should pass validation if "options.validate.schema" is false', async () => {
    ...
    const api = await SwaggerParser.validate(path.rel("test/specs/validate-schema/invalid/" + invalid.file), {
  ...
  for (const test of tests) {
    if (test.valid) {
      it(test.name, async () => {
        try {
          const api = await SwaggerParser.validate(path.rel("test/specs/validate-schema/valid/" + test.file));
          expect(api).to.be.an("object");
        } catch (err: any) {
      ...
      it(test.name, async () => {
        try {
          await SwaggerParser.validate(path.rel("test/specs/validate-schema/invalid/" + test.file));
