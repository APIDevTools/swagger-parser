...
function validateSpec (api) {
  ...
  definitions.forEach(function (definitionName) {
    ...
    validateRequiredPropertiesExist(definition);
  });
...
function validateParameterTypes (params, api, operation, operationId) {
  params.forEach(function (param) {
    ...
    validateRequiredPropertiesExist(schema);
 ...
 * @param {object}    schema      - A Schema object, from the Swagger API
 */
function validateRequiredPropertiesExist (schema) {
  if (schema.required && Array.isArray(schema.required)) {
    schema.required.forEach(function (requiredProperty) {
      if (!(schema.properties && schema.properties[requiredProperty])) {
        throw ono.syntax('Validation failed. Required property \'%s\' does not exist in object', requiredProperty);
      }
    });
  }
}
