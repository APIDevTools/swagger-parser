  ...
  for (let pathName of paths) {
    ...
    if (api.openapi) {
      ...
      for (let schemaName of schemaNames) {
        ...
        try {
          validateSchema(schema, schemaId, schemaTypes);
        }
        catch (err) {
          message += err.message;
          isValid = false;
      ...
      for (let definitionName of definitions) {
        ...
        try {
          validateSchema(definition, definitionId, schemaTypes);
        }
        catch (err) {
          message += err.message;
          isValid = false;
      ...
      for (let contentType of contentTypes) {
        ...
        if (content.schema) {
          ...
          if (validTypes.indexOf(content.schema.type) === -1) {
            ...
            try {
              validateSchema(content.schema, responseId + "/content/" + contentType + "/schema", validTypes);
            }
            catch (err) {
              message += err.message;
              isValid = false;
...
function validateSchema (schema, schemaId, validTypes) {
  ...
  // make sure that array's have items defined
  ...
  // Recursively check all the properties...
  let propNames = Object.keys(schema.properties || {});
  for (let propName of propNames) {

    try {
      validateSchema(schema.properties[propName], schemaId + "/" + propName, schemaTypes);
    }
    catch (err) {
      message += err.message;
      isValid = false;
